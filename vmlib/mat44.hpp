...
{
	float const cos = std::cos(aAngle);
	float const sin = std::sin(aAngle);
...
{
	float const tanHalfFov = std::tan(aFovInRadians * 0.5f);
	float const f = 1.f / tanHalfFov; // focal length
	float const d = aNear - aFar;     // distance between near and far planes

	return Mat44f{
		f / aAspect, 0.f,  0.f,                   0.f,
		0.f,       f,    0.f,                   0.f,
		0.f,       0.f,  (aFar + aNear) / d,          2.f * aFar * aNear / d,
		0.f,       0.f,  -1.f,                  0.f
	};
}
